function kmlStr = ge_surf(x,y,z,InputColorMap,varargin)
%
...
if cLimLow==cLimHigh
    disp('Data is all one value in ge_surf.')
end
...
YBlue = C(:,3);
kmlStr='';
for lat=2:length(y(:,1))-1
    disp(['kml processing: ' num2str(100*lat/length(y(:,1))-1) '% done.'  ])
    for lon=2:length(x(1,:))-1
        ...
        if ~isnan(z(lat,lon))
            if isequal(polyAlpha,'00')
                ...
                f = (z(lat,lon)-cLimLow)/(cLimHigh-cLimLow);
                if f<0
                    f=0;
                end
                if f>1
            ...
            xv=[x(lat,lon)-halfLonRes;x(lat,lon)+halfLonRes;x(lat,lon)+halfLonRes;x(lat,lon)-halfLonRes];
            yv=[y(lat,lon)-halfLatRes;y(lat,lon)-halfLatRes;y(lat,lon)+halfLatRes;y(lat,lon)+halfLatRes];
            zv=det_altitude(x,y,z,lat,lon,vertExagg,halfLonRes,halfLatRes,altRefLevel);

               kmlStr = [kmlStr,ge_poly3(xv,yv,zv,...
                                           'snippet', snippet,...
                                           'description', description, ...
                                           'name',['row=',num2str(lat),';col=',num2str(lon)],...
